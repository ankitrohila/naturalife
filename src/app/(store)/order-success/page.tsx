...
export default async function OrderSuccessPage({ searchParams }: { searchParams: Promise<{ id?: string }> }) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center py-20 px-4" style={{ backgroundColor: 'var(--ivory)' }}>
      <div className="max-w-md w-full bg-white rounded-none border border-gray-100 shadow-sm p-10 text-center">
        ...
        <div className="flex flex-col gap-3">
          {id && (
            <a href={`/api/invoices/${id}?print=1`} target="_blank" rel="noopener noreferrer" className="py-3 text-white rounded-none font-semibold" style={{ backgroundColor: 'var(--green)' }}>
              Download Invoice
            </a>
          )}
          <Link href="/account" className="py-3 rounded-none font-semibold border border-[var(--green)]" style={{ color: 'var(--green)' }}>
            View My Orders
          </Link>
          <Link href="/shop" className="py-3 border border-gray-300 rounded-none font-semibold text-gray-700 hover:bg-gray-50">
            Continue Shopping
          </Link>
        </div>
      </div>
